...
class Parser {
  constructor(editor, buffer, options) {
    ...
    // Find tags in buffer
    this.matchSingleComments();
    this.matchBlockComments();
    this.matchDocComments();
  ...
  matchBlockComments() {
    ...
    // First find rows of comments matching the block comments
  ...
  matchDocComments() {
    ...
    let comments = `(^)+([ \\t]*\\*[ \\t]*)(${characters})([ ]*|[:])+([^*/][^\\r\\n]*)`;
    let commentRegExp = new RegExp(comments, "igm");

    // First find rows of comments matching pattern /** */
    this.FindComments(regExp, this.buffer, m => {
      this.FindComments(commentRegExp, m.range, this.onMatch);
    });
  }
